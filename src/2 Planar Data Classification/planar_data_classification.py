...
print (f'\nAccuracy of logistic regression: {accuracy}% (percentage of correctly labelled datapoints)')
...
print(f"\ncost = {str(compute_cost(A2, Y_assess, parameters))}")
...
def backward_propagation(parameters, cache, X, Y):
    ...
    dZ2 = A2 - Y
    dW2 = dZ2.dot(np.transpose(A1)) / m
    db2 = np.sum(dZ2, axis=1, keepdims=True) / m
    dZ1 = np.transpose(W2).dot(dZ2) * (1 - np.power(A1, 2))
    dW1 = dZ1.dot(np.transpose(X))
...
print (f'dW2 = {str(grads["dW2"])}')
print (f'db2 = {str(grads["db2"])}')
...
print(f"\nPredictions mean = {str(np.mean(predictions))}")
